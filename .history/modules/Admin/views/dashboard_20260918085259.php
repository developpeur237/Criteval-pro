  ...
  <div class="modal" style="max-width:520px">
    ...
    <form id="planningForm">
      <div class="modal-body"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><div style="display:grid;gap:14px">
        ...
        <label class="form-label">Organisation liée<select name="project_id" class="form-input-dark" style="width:100%"><option value="">Organisation du formulaire</option><?php foreach (($projects ?? []) as $planningProject): ?><option value="<?= (int) $planningProject['id'] ?>"><?= e($planningProject['organization'] ?: $planningProject['title']) ?></option><?php endforeach; ?></select></label>
        ...
        <label class="form-label">Pays éligibles<select name="allowed_countries[]" class="form-input-dark" style="width:100%;min-height:82px" multiple><?php foreach ($planningCountries as $country => $_): ?><option value="<?= e($country) ?>"><?= e($country) ?></option><?php endforeach; ?></select><small class="settings-desc">Aucun pays sélectionné = tous les pays autorisés.</small></label>
