...
function renderSidebarCounts() {
  ...
  liveItems.forEach(item => {
    ...
    const seenAt = notificationSeenAt(item);
    const isRead = seenAt > 0 && !!readMeta[item.id] && !isNotificationExpired(item.id, readMeta[item.id], item.module || 'apercu');
    if (!isRead) {
...
function renderNotificationList(items) {
  ...
  const unreadItems = visibleItems.filter(item => {
    ...
    return isNotificationExpired(item.id, readAt, item.module || 'apercu');
  });
