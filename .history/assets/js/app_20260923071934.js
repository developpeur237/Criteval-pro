...
function initCalendar() {
  ...
  $(calEl).fullCalendar({
    ...
    events: function (start, end, timezone, callback) {
      fetch((window.CRITEVAL_BASE_URL || '') + '/admin/forms/schedules')
        .then(response => response.ok ? response.json() : [])
        .then((items) => {
          const mapped = (Array.isArray(items) ? items : []).map(function (item) {
            if (item && item.type === 'training') {
              return {
            ...
            const title = (item.form_title || 'Formulaire') + (item.project_title ? ' — ' + item.project_title : '');
            return {
              title: title,
    ...
    eventClick: function(event) { showToast('Planning : ' + event.title, 'success'); },
