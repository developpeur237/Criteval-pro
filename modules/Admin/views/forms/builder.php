...
<main class="builder-page">
  ...
  <form method="post" action="<?= BASE_URL ?>/admin/forms" id="form-builder">
    ...
    <div class="builder-meta"><label>Titre <input required name="title" value="<?= e($form['title'] ?? '') ?>"></label><label>Projet <select name="project_id"><option value="">Aucun</option><?php foreach ($projects as $project): ?><option value="<?= (int) $project['id'] ?>" <?= (int) ($form['project_id'] ?? 0) === (int) $project['id'] ? 'selected' : '' ?>><?= e($project['title']) ?></option><?php endforeach; ?></select></label><label>Statut <select name="status"><?php foreach (['draft'=>'Brouillon','published'=>'Publié','archived'=>'Archivé'] as $key=>$label): ?><option value="<?= $key ?>" <?= ($form['status'] ?? 'draft') === $key ? 'selected' : '' ?>><?= $label ?></option><?php endforeach; ?></select></label></div>
