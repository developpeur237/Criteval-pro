...
function renderNotificationList(items) {
  ...
  count.title = items.length + ' notification(s) disponible(s), dont ' + unreadItems.length + ' non lue(s)';
...
async function refreshNotifications() {
  try {
    ...
    if (notificationKnownStates && items.some(item => notificationKnownStates.get(item.id) !== (item.state || 'pending'))) {
      ...
      if (payload.browser_alerts && 'Notification' in window && Notification.permission === 'granted') {
        const changedItem = items.find(item => notificationKnownStates.get(item.id) !== (item.state || 'pending'));
