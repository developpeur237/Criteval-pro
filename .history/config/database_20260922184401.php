...
function reset_database_to_initial_state(): string
{
    ...
    $temporaryPath = DATABASE_PATH . '.reset-' . bin2hex(random_bytes(8)) . '.sqlite';
    try {
        $temporary = create_pdo_connection(['path' => $temporaryPath], true);
        $temporary = null;

        if (is_file(DATABASE_PATH) && !unlink(DATABASE_PATH)) {
            throw new RuntimeException('Impossible de remplacer la base SQLite actuelle.');
        }
        if (!rename($temporaryPath, DATABASE_PATH)) {
            throw new RuntimeException('Impossible d’installer la base SQLite initiale.');
        }
    } catch (Throwable $exception) {
