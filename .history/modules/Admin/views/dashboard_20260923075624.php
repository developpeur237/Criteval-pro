...
function isNotificationExpired(itemId, readAt = null) {
  const value = Number(readAt ?? notificationReadMeta()[itemId] ?? 0);
...
function renderSidebarCounts() {
  ...
  liveItems.forEach(item => {
    ...
    const isRead = readAt && !isNotificationExpired(item.id, readAt);
    if (!isRead) {
      const module = item.module || 'apercu';
      unreadByModule[module] = (unreadByModule[module] || 0) + 1;
    }
  });
...
function renderNotificationList(items) {
  ...
  const visibleItems = items.filter(item => {
    ...
    return (Date.now() - Number(readAt)) <= (5 * 60 * 1000);
  });
