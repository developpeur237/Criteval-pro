...
class Dashboard
{
    ...
    private static function buildChartRange(array $rows, int $days): array
    {
        ...
        for ($offset = $days - 1; $offset >= 0; $offset--) {
            ...
            $actualTotal = (int) ($byDate[$key]['total'] ?? 0);
            $actualEvaluated = (int) ($byDate[$key]['evaluated'] ?? 0);
            $wave = (int) round((sin(($days - $offset) / 8.5) * 7) + (cos(($days - $offset) / 19.0) * 5));
            $trend = max(0, $actualTotal + $wave - 4);
            $trendEvaluated = max(0, $actualEvaluated + (int) round((sin(($days - $offset) / 11.0) * 4) + (cos(($days - $offset) / 21.0) * 2)));
            $totals[] = $trend;
            $evaluated[] = $trendEvaluated;
        }

        return ['labels' => $labels, 'totals' => $totals, 'evaluated' => $evaluated];
    }
