...
class Notification
{
    public static function current(array $settings): array
    {
        ...
        if (!empty($settings['notifications']['schedule_alerts'])) {
            $schedules = db()->query(
                'SELECT s.id, s.start_datetime, s.end_datetime, s.is_active, s.created_at,
            ...
            foreach ($schedules as $schedule) {
                if (!self::isRecentlyAdded($schedule['created_at'] ?? null)) {
                    continue;
                }
        ...
        $sessions = db()->query(
            'SELECT t.id, t.name, t.session_date, t.created_at, p.title AS project_title
             FROM training_sessions t LEFT JOIN projects p ON p.id = t.project_id
             WHERE t.is_active = 1 ORDER BY t.session_date ASC'
        )->fetchAll();
        foreach ($sessions as $session) {
            if (!self::isRecentlyAdded($session['created_at'] ?? null)) {
                continue;
            }
        ...
        foreach ((array) ($settings['notifications']['custom'] ?? []) as $custom) {
            ...
            if (!self::isRecentlyAdded($custom['created_at'] ?? ($custom['due_at'] ?? null))) {
                continue;
            }
    ...
    private static function isRecentlyAdded(?string $createdAt): bool
    {
