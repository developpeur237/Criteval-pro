...
function renderSidebarCounts() {
  ...
  if (!liveItems.length) {
    Object.keys(sidebarModuleCounts || {}).forEach(module => {
      const item = document.querySelector('.sidebar-item[onclick*="switchModule(\'" + module + "\')"]');
      if (!item) return;
      ...
      if (!badge) {
        ...
        newBadge.textContent = count > 99 ? '99+' : String(count);
        return;
      }
      badge.textContent = count > 99 ? '99+' : String(count);
    });
