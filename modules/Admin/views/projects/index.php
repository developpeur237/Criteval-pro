...
<main class="stub-page admin-stub">
  ...
  <form method="post" action="<?= BASE_URL ?>/admin/organisations" class="admin-form" enctype="multipart/form-data">
    ...
    <div class="form-grid">
      ...
      <label>Nombre de projets de l’organisation<input type="number" min="0" name="project_count" value="<?= e((string) ($editProject['project_count'] ?? '')) ?>"></label>
      <label>Zone d’intervention de l’organisation<input name="intervention_zone" value="<?= e($editProject['intervention_zone'] ?? '') ?>"></label>
      ...
      <label>Durée d’activité (mois)<input type="number" min="1" name="duration_months" value="<?= e((string) ($editProject['duration_months'] ?? '')) ?>"></label>
