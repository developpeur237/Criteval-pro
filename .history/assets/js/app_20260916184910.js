...
const submissionsChartData = {
  ...
  submissions: [8, 10, 14, 18, 12, 16, 12, 19, 28, 45, 67, 92],
  evaluated: [6, 8, 11, 14, 10, 13, 10, 15, 22, 38, 54, 80]
};
const submissionChartPeriods = { '1m': 1, '3m': 3, '6m': 6, '1a': 12, tout: 12 };
...
function initCharts() {
  ...
  if (ctx1) {
    chartInstances.submissions = new Chart(ctx1, {
      type: 'line',
      data: {
        labels: submissionsChartData.labels.slice(-6),
        datasets: [{
          label: 'Soumissions',
          data: submissionsChartData.submissions.slice(-6),
          borderColor: '#2EAF7D',
          ...
          label: 'Évaluées',
          data: submissionsChartData.evaluated.slice(-6),
          borderColor: '#F5A623',
...
function updateSubmissionsChart(period) {
  ...
  chart.data.labels = submissionsChartData.labels.slice(-count);
  chart.data.datasets[0].data = submissionsChartData.submissions.slice(-count);
  chart.data.datasets[1].data = submissionsChartData.evaluated.slice(-count);
  chart.update();
