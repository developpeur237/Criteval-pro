...
try {
    ...
    for ($index = $existingHistorical; $index < $target; $index++) {
        ...
        $submittedAt = (new DateTimeImmutable('now'))
            ->modify('-' . mt_rand(0, 560) . ' days')
            ->setTime(mt_rand(7, 20), mt_rand(0, 59), mt_rand(0, 59));
        ...
        foreach (array_slice($criteria, 0, max(3, min(6, count($criteria)))) as $criterion) {
            ...
            $evaluatedAt = $submittedAt->modify('+' . mt_rand(1, 21) . ' days');
        ...
        $insertResult->execute([
            ...
            'published' => $published,
            'published_at' => $published ? $submittedAt->modify('+22 days')->format('Y-m-d H:i:s') : null,
        ]);
    }
    ...
    for ($index = $scheduleCount; $index < 18; $index++) {
        $form = $forms[$index % count($forms)];
        $start = (new DateTimeImmutable('now'))->modify('-' . (540 - ($index * 27)) . ' days')->setTime(9, 0);
