        ...
        <div class="chart-card">
          <div class="chart-card-header">
            ...
            <div class="chart-actions">
              ...
              <button class="chart-btn active" data-period="6m" onclick="updateSubmissionsChart('6m')">6M</button>
...
let chartInstances = {};
const submissionChartPeriods = { '1m': 1, '3m': 3, '6m': 6, '1a': 12, tout: null };

function updateSubmissionsChart(period) {
  const chart = chartInstances.submissions;
  const count = submissionChartPeriods[period];
  const labels = overviewData.monthly?.labels || [];
  const submissions = overviewData.monthly?.totals || [];
  if (!chart || count === undefined) return;

  const start = count ? Math.max(labels.length - count, 0) : 0;
  chart.data.labels = labels.slice(start);
  chart.data.datasets[0].data = submissions.slice(start);
  chart.data.datasets[1].data = submissions.slice(start).map(value => Math.round(value * 0.7));
  chart.update();
...
function initCharts() {
  ...
  if (ctx1) {
    chartInstances.submissions = new Chart(ctx1, {
      type: 'line',
      data: {
        labels: (overviewData.monthly?.labels || []).slice(-6),
        datasets: [{
          label: 'Soumissions',
          data: (overviewData.monthly?.totals || []).slice(-6),
          borderColor: '#2EAF7D',
          ...
          label: 'Évaluées',
          data: (overviewData.monthly?.totals || []).slice(-6).map(value => Math.round(value * 0.7)),
          borderColor: '#F5A623',
