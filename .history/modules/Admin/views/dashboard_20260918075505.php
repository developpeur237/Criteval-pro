  ...
  <header class="topbar" id="topbar">
    ...
    <div class="topbar-actions">
      ...
      <div class="notification-panel" id="notificationPanel" hidden>
        <div class="notification-panel-header"><span>Notifications</span><span style="display:flex;align-items:center;gap:10px"><button type="button" class="notification-mark-all" id="notificationMarkAll">Tout marquer comme lu</button><small id="notificationUpdatedAt">Actualisation...</small></span></div>
    ...
    <div class="dash-module" id="module-projets">
      ...
      <div class="filter-bar organisation-filter-bar" data-organisation-filters>
        ...
        <select class="filter-select" data-organisation-sort aria-label="Trier les organisations">
          <option value="date">Trier par date</option><option value="name">Trier par nom</option><option value="projects">Trier par projets</option>
        </select>
      </div>
...
async function refreshNotifications() {
  try {
    ...
    if (notificationKnownIds && items.some(item => !notificationKnownIds.has(item.id))) {
      ...
      if (payload.browser_alerts && 'Notification' in window && Notification.permission === 'granted') {
        new Notification('Criteval Pro', { body: items.find(item => !notificationKnownIds.has(item.id)).message });
      }
    }
    notificationKnownIds = ids;
    window.currentNotificationItems = items;
    renderNotificationList(items);
    ...
    const list = document.getElementById('notificationList');
    if (list && !notificationKnownIds) list.innerHTML = '<div class="notification-empty">Alertes temporairement indisponibles.</div>';
  }
}
