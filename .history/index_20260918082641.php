...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $route === 'admin/forms/schedules') {
    ...
    $allowedCountries = Form::countryList($_POST['allowed_countries'] ?? '');
...
if ($route === 'otp' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $availability = $formId > 0 ? Form::availability($formId) : ['available' => false, 'message' => 'Formulaire invalide.'];
...
if ($route === 'candidate/submit' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $availability = Form::availability($formId, $_POST['country_code'] ?? null);
    if (!$availability['available']) {
        http_response_code(422);
        echo json_encode(['success' => false, 'message' => $availability['message']]);
        exit;
