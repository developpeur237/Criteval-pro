  ...
  (function () {
    ...
    const initCriteriaModule = function () {
      ...
      window.editCriterion = async function (criterion) {
        const label = window.prompt('Libellé du critère', criterion.label || '');
        if (label === null || !label.trim()) return;
        const formData = new FormData();
        formData.append('id', String(criterion.id));
        formData.append('label', label.trim());
        formData.append('description', criterion.description || '');
        formData.append('weight', String(criterion.weight || 1));
        formData.append('max_score', String(criterion.max_score || 20));
        formData.append('is_required', criterion.is_required ? '1' : '0');
        formData.append('csrf_token', criteriaForm.querySelector('input[name="csrf_token"]').value);
        try {
