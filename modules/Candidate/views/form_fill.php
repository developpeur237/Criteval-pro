...
<script>(function(){const fs=document.getElementById('formId'),custom=document.getElementById('customFields');if(!fs||!custom)return;const catalog=<?= json_encode($catalog,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES) ?>;const px=v=>Number(v||0)+'px';const color=v=>/^#[0-9a-f]{6}$/i.test(v||'')?v:'';function paint(){let l=[];try{l=JSON.parse((catalog[fs.value]?.form?.layout_json)||'[]')}catch(e){}if(l&&l.fields)l=l.fields;l=(Array.isArray(l)?l:[]).filter(x=>!['heading','paragraph','divider','section','file'].includes(x.type));Array.from(custom.children).forEach((d,i)=>{const x=l[i];if(!x)return;const span=Math.max(1,Math.min(12,Number(x.span)||6));d.style.gridColumn=x.gridColumnStart?(x.gridColumnStart+' / span '+span):'span '+span;d.style.gridRow=x.gridRowStart?(x.gridRowStart+' / span '+Math.max(1,Number(x.rowSpan)||1)):'span '+Math.max(1,Number(x.rowSpan)||1);d.style.width=x.widthUnit==='px'?px(x.widthValue):x.widthUnit==='auto'?'auto':(Number(x.widthValue||100)+'%');d.style.minWidth=x.minWidth?px(x.minWidth):'0';d.style.maxWidth=x.maxWidth?px(x.maxWidth):'none';d.style.minHeight=x.minHeight?px(x.minHeight):'0';d.style.maxHeight=x.maxHeight?px(x.maxHeight):'none';d.style.margin=[x.marginTop,x.marginRight,x.marginBottom,x.marginLeft].map(px).join(' ');d.style.padding=[x.paddingTop,x.paddingRight,x.paddingBottom,x.paddingLeft].map(px).join(' ');d.style.position=x.position==='normal'?'relative':(x.position||'relative');d.style.left=x.position==='absolute'?px(x.offsetX):'auto';d.style.top=x.position==='absolute'?px(x.offsetY):'auto';d.style.zIndex=Number(x.zIndex)||1;d.style.opacity=Number(x.opacity??100)/100;d.style.border=(Number(x.borderWidth??1)+'px '+(x.borderStyle||'solid')+' '+(color(x.borderColor)||'#d5dce7'));d.style.borderRadius=[x.radiusTopLeft??x.radius,x.radiusTopRight??x.radius,x.radiusBottomRight??x.radius,x.radiusBottomLeft??x.radius].map(px).join(' ');d.style.background=x.backgroundType==='gradient'&&String(x.backgroundGradient||'').startsWith('linear-gradient')?x.backgroundGradient:(color(x.background)||'#fff');d.style.color=color(x.color)||'#1e293b';d.style.textAlign=x.textAlign||'left';d.style.fontSize=px(x.fontSize||14);d.style.fontWeight=x.fontWeight||600;d.style.lineHeight=(Number(x.lineHeight)||140)/100;d.style.letterSpacing=px(x.letterSpacing);d.style.visibility=x.visibility||'visible';d.style.overflow=x.overflow||'visible';d.style.transform='translate('+(x.position==='absolute'?0:Number(x.offsetX)||0)+'px,'+(x.position==='absolute'?0:Number(x.offsetY)||0)+'px) scale('+(Number(x.scale||100)/100+') rotate('+(Number(x.rotate)||0)+'deg) skew('+(Number(x.skewX)||0)+'deg,'+(Number(x.skewY)||0)+'deg)');d.style.transformOrigin=x.transformOrigin||'center';d.style.alignSelf=x.alignSelf||'stretch';d.style.justifySelf=x.justifySelf||'stretch';d.querySelectorAll('input,select,textarea').forEach(c=>{c.style.background=color(x.inputBackground)||'#fff';c.style.borderColor=color(x.inputBorderColor)||'#d5dce7';c.style.borderRadius=px(x.inputRadius??8);c.style.padding=px(x.inputPadding??12);c.style.accentColor=color(x.accentColor)||'#2eaf7d'})})}new MutationObserver(paint).observe(custom,{childList:true});fs.addEventListener('change',()=>setTimeout(paint,0));paint()})();</script>
<script>(function(){const fs=document.getElementById('formId'),custom=document.getElementById('customFields');if(!fs||!custom)return;const catalog=<?= json_encode($catalog,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES) ?>;function fit(){let l=[];try{l=JSON.parse(catalog[fs.value]?.form?.layout_json||'[]')}catch(e){}if(l&&l.fields)l=l.fields;l=(Array.isArray(l)?l:[]).filter(x=>!['heading','paragraph','divider','section','file'].includes(x.type));Array.from(custom.children).forEach((d,i)=>{const x=l[i];if(x)d.style.gridColumn=(Number(x.span||12)>12?'span 2':'span 1')})}new MutationObserver(fit).observe(custom,{childList:true});fs.addEventListener('change',()=>setTimeout(fit,0));fit()})();</script>
</body></html>
