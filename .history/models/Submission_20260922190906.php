...
class Submission
{
    ...
    {
        $firstSubmission = db()->query(
            "SELECT MIN(submitted_at) FROM submissions WHERE submitted_at <= datetime('now')"
        )->fetchColumn();
        if ($firstSubmission === false || $firstSubmission === null) {
            return [];
        }
    ...
    public static function countryCounts(int $limit = 5): array
    {
        $stmt = db()->prepare(
            ...
            SELECT country_code, SUM(total) AS total
