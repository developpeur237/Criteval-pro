...
class Notification
{
    public static function current(array $settings): array
    {
                ...
                if ($start !== null) {
                    ...
                    if ($seconds >= 0 && $seconds <= $leadSeconds) {
                        $notifications[] = self::item(
                            ...
                            'Ouverture imminente',
                            $prefix . ' ouvre ' . self::relativeTime($seconds) . '.',
                            $start,
                            'calendrier'
                        );
                    }
        ...
        foreach ($sessions as $session) {
            ...
            if ($seconds >= 0 && $seconds <= $trainingLeadDays * 86400) {
                $notifications[] = self::item(
                    'training-' . (int) $session['id'],
                    'training',
                    $seconds <= 86400 ? 'urgent' : 'info',
                    'Formation programmée',
                    (string) $session['name'] . ' est prévue ' . self::relativeTime($seconds) . '.',
                    $date,
                    'formation'
                );
            }
    ...
    private static function item(string $id, string $type, string $severity, string $title, string $message, DateTimeImmutable $dueAt, string $module): array
    {
        return [
            'id' => $id,
            'type' => $type,
